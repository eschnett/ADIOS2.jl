...
if comm_rank == 0
    ...
    @testset "High-level read tests " begin
        ...
        v1 = adios_get(file, "v1"; start=(1,))
        @test !isready(v1)
        @test fetch(v1) == fill(float(ℯ), 1)
        @test isready(v1)
        v3 = adios_get(file, "v3"; start=(1,))
        v4 = adios_get(file, "v4"; start=(1, 1))
        @test !isready(v3)
        @test fetch(v3) == makearray(1, float(ℯ))
        @test fetch(v4) == makearray(2, float(ℯ))
        @test isready(v3)
        v5 = adios_get(file, "v5"; start=(1, 1, 1))
        v6 = adios_get(file, "g1/v6"; start=(1, 1, 1, 1))
        v7 = adios_get(file, "g1/g2/v7"; start=(1, 1, 1, 1, 1))
        @test !isready(v5)
        adios_perform_gets(file)
        @test isready(v5)
        @test fetch(v5) == makearray(3, float(ℯ))
        ...
        v1 = adios_get(file, "v1"; count=(1,))
        @test !isready(v1)
        @test fetch(v1) == fill(float(ℯ), 1)
        @test isready(v1)
        v3 = adios_get(file, "v3"; count=(1,))
        v4 = adios_get(file, "v4"; count=(1, 1))
        @test !isready(v3)
        @test fetch(v3) == makearray(1, float(ℯ))
        @test fetch(v4) == makearray(2, float(ℯ))
        @test isready(v3)
        v5 = adios_get(file, "v5"; count=(1, 1, 1))
        v6 = adios_get(file, "g1/v6"; count=(1, 1, 1, 1))
        v7 = adios_get(file, "g1/g2/v7"; count=(1, 1, 1, 1, 1))
        @test !isready(v5)
        adios_perform_gets(file)
        @test isready(v5)
        @test fetch(v5) == makearray(3, float(ℯ))
        ...
        v1 = adios_get(file, "v1"; start=(1,), count=(1,))
        @test !isready(v1)
        @test fetch(v1) == fill(float(ℯ), 1)
        @test isready(v1)
        v3 = adios_get(file, "v3"; start=(1,), count=(1,))
        v4 = adios_get(file, "v4"; start=(1, 1), count=(1, 1))
        @test !isready(v3)
        @test fetch(v3) == makearray(1, float(ℯ))
        @test fetch(v4) == makearray(2, float(ℯ))
        @test isready(v3)
        v5 = adios_get(file, "v5"; start=(1, 1, 1), count=(1, 1, 1))
        v6 = adios_get(file, "g1/v6"; start=(1, 1, 1, 1), count=(1, 1, 1, 1))
        v7 = adios_get(file, "g1/g2/v7"; start=(1, 1, 1, 1, 1), count=(1, 1, 1, 1, 1))
        @test !isready(v5)
        adios_perform_gets(file)
        @test isready(v5)
        @test fetch(v5) == makearray(3, float(ℯ))
